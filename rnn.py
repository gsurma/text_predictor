...
def log(smooth_loss, losses):
    iteration = len(losses)
    print('{{"metric": "iteration", "value": {}}}'.format(iteration-1))
...
def rnn():
    ...
    while True:
        ...
        for param, gradient_param, memory in zip([input_to_hidden, hidden_to_hidden, hidden_to_output, hidden_bias, output_bias],
            ...
            param += -LEARNING_RATE * gradient_param / np.sqrt(memory + ADAGRAD_UPDATE_RATE)  # adagrad update
